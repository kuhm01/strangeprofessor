...
### 정적변수 규약
7개의 정적변수는 내부규칙에 따라 정해진 규약이 있습니다.</br>
+ Professor[0] : 분기 판별에 사용
+ Professor[1] : 분기 판별에 사용
+ Professor[6] : 미니버퍼 연산 실행값 저장
---

1, 2번째 정적변수는 그 저장 된 값이 분기 판별에 사용됩니다.</br>
상세한 내용은 후술.</br>
마지막 정적변수는 미니버퍼에서 실행된 연산의 결과값이 저장됩니다.</br>
결과값이 새로이 Node를 생성하여 저장되므로 기존 값은 유실됨에 유의하세요.</br>
...
### 미니버퍼
구현체는 미니버퍼를 가집니다.</br>
이 것은 두 개의 정적변수공간으로</br>
이상한 나라의 교수님은 저장 된 두 개의 값으로</br>
덧셈과 곱셈을 시행하게됩니다.</br>
...
### 평점곱, 합
평점곱, 평점합 키워드를 사용하면 minibuffer에 저장된 두 값을 바탕으로</br>
각 곱연산, 합연산을 실행하여 그 결과값을 마지막 교수님에 저장합니다.</br>
즉, 7개의 정적변수 공간 중 마지막 공간에 저장된다는 이야기입니다.</br>
...
반드시 통과해야 합니다.</br>
[다음](../main/TESTCASE.md "test case")에서 소개
